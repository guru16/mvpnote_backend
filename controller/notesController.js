...
async createSubject(req){
    ...
    async getNotes(req){
        if(req.userId){
            ...
            let result = await notesModal.find({$and:[{userId:ObjectId(req.userId)},{isDeleted:false}] })
    ...
    async getNotesBySubject(subjectId){
        ...
        const result = await notesModal.find({  $and: [ {subjectId:ObjectId(subjectId) },{isDeleted:false}]}).populate('subjectId')
           ...
           console.log(notesId)
            ...
            if(notes){
               ...
               // console.log(finalResult)
                if(finalResult){                    
                    return ({ status: 200, message:'notes deleted successfully ', success: false, data:"" });
    ...
    async updateNotes(req){
        try{
            ...
            if(req.body && req.body.noteId && req.body.noteTitle){
                let notes =await notesModal.findOne({ _id:ObjectId(req.body.noteId )});
                if(notes){
                    notes.notesName=req.body.noteTitle
                    let finalResult= await notes.save()
                    if(finalResult){
                        return ({ status: 200, message: 'notes updates successfully', success: true, data: finalResult });
                    ...
                    return ({ status: 400, message: 'this notes is not exsist ', success: false, data: '' });
    ...
    async getDeletedData(req){
        console.log(req.userId)
        try{
            if(req.userId){
                const result = await SubjectsModal.find({  $and: [ {userId:ObjectId(req.userId) }, { isDeleted:true }]})
                
                const result1 = await notesModal.find({  $and: [ {userId:ObjectId(req.userId) }, { isDeleted:true }]})
               var subjectIdArray = [];
