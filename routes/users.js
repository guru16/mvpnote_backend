...
router.post('/updateNotes',authMiddleware.authenticateJWT,function(req,res,next){
  return notesController.updateNotes(req)
  .then((resp)=>{
    res.send(resp);
  })
  .catch(err=>{
...
router.get('/getDeletedData',authMiddleware.authenticateJWT,function(req,res,next){
  return notesController.getDeletedData(req)
  .then((resp)=>{
    res.send(resp);
